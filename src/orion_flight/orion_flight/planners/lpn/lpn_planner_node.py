#!/usr/bin/env python3
"""Linearized Proportional Navigation (LPN) planner ROS2 node."""
...
from orion_flight.planners.lpn.lpn_algorithm import LinearizedPNAlgorithm


class LPNPlannerNode(PlannerBase):
    """
    ROS2 node for Linearized Proportional Navigation (LPN) guidance law.
    
    Subscribes to interceptor and target states, computes LPN guidance,
    and publishes trajectory setpoints to PX4.
    """
    
    def __init__(self):
        super().__init__('lpn_planner')
        
        # Declare ROS parameters
        self.declare_parameter('G_lpn', 20.0)
        ...
        # Get parameters
        params = {
            'G_lpn': self.get_parameter('G_lpn').value,
        ...
        # Initialize LPN algorithm
        self.algorithm = LinearizedPNAlgorithm(params)
        ...
        self.get_logger().info(
            f'LPN Planner initialized: G_lpn={params["G_lpn"]}, '
            f'min_tgo={params["min_tgo"]}, amax={params["amax"]}, '
    ...
    def compute_guidance(self, planner_input: PlannerInput) -> PlannerOutput:
        ...
        return PlannerOutput(
            ...
            commanded_yaw=0.0,  # LPN doesn't specify yaw
    ...
    def reset(self):
        ...
        self.get_logger().info('LPN Planner reset')
...
def main(args=None):
    """Main entry point for LPN planner node."""
    ...
    try:
        ...
        node.get_logger().info('LPN Planner shutting down...')
    finally:
        node.destroy_node()
        rclpy.shutdown()
