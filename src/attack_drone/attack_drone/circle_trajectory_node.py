...
class CircleTrajectoryNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.flight_phase = "INIT"  # INIT -> TAKEOFF -> HOVER -> COMPLETE
        ...
        self.get_logger().info('Hover Flight Node Started!')
        self.get_logger().info(f'Parameters: hover_height={-self.flight_height}m')
    ...
    def timer_callback(self) -> None:
        ...
        if self.flight_phase == "INIT":
            ...
            if self.vehicle_local_position is not None:
                ...
                # Transition to HOVER when at target altitude (altitude_error already calculated above)
                if self.is_armed and self.is_offboard and altitude_error < 0.2:  # Within 20cm of target
                    self.flight_phase = "HOVER"
                    self.get_logger().info(f"✓ Reached target height! Current z={current_z:.2f}m, Target z={target_z:.2f}m. Starting hover mode.")
            else:
                if self.offboard_setpoint_counter % 20 == 0:
                    self.get_logger().warn("[TAKEOFF] Waiting for vehicle position data...")

        elif self.flight_phase == "HOVER":
            # Publish hover position (stay at initial location)
            self.publish_position_setpoint(0.0, 0.0, self.flight_height)
            ...
            # Debug output every 2 seconds
            if self.offboard_setpoint_counter % 20 == 0:
                self.get_logger().info(f'[HOVER] Hovering at height {-self.flight_height}m, Position: x=0.0, y=0.0, z={self.flight_height}')
